...
function acessar() {
  ...
  if (erros.length > 0) {
    for (let posicao = 0; posicao < erros.length; posicao++) {
      document.getElementById(erros[posicao].id).innerHTML =
        erros[posicao].message;
      document
        .getElementById(
          erros[posicao].id.substring(0, erros[posicao].id.length - 5)
        )
...
function verificarDados() {
  ...
  for (
    let posicaoAtual = 0;
    posicaoAtual < bancoCadastrado.agencias.length;
    posicaoAtual++
  ) {
  ...
  for (
    let posicaoAtual = 0;
    posicaoAtual < agenciaEncontrada?.contas.length;
    posicaoAtual++
  ) {
